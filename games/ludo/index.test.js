...
describe('User make full move', () => {
  ...
  test('Player rolls 5 with pawns on spawn', () => {
    ...
    const stopProgressAction = returnActions[1];
    const waitAction = returnActions[2];
    expect(rollAction.action.type).toBe(Ludo.ActionTypes.Roll);
    expect(rollAction.action.diceNumber).toBe(5);
    expect(waitAction.type).toBe(Ludo.ActionTypes.WaitForPlayer);
    expect(waitAction.playerId).toBe(roomState.playerIds[1]);
    expect(waitAction.expectedAction).toBe(Ludo.ActionTypes.Roll);
    ...
    currentTime++;
    const callbackActions = rollAction.callback();
  ...
  test('Player rolls 6 with pawns on spawn', () => {
    ...
    expect(rollAction.timestamp).toBe(currentTime + Ludo.AnimationLengths.rollDice);
    expect(isFunction(rollAction.callback)).toBe(true);
    expect(stopProgressAction.action.type).toBe(Ludo.ActionTypes.StopProgress);

    currentTime++;
    const callbackActions = rollAction.callback();
    expect(roomState.rolled).toBe(true);
    expect(callbackActions.length).toBe(3);
    const selectPawnsAction = callbackActions[0].action;
    const waitAction = callbackActions[1].action;
    const resetProgressAction = callbackActions[2].action;
    ...
    expect(waitAction.expectedAction).toBe(Ludo.ActionTypes.PickPawn);
    expect(resetProgressAction.type).toBe(Ludo.ActionTypes.RestartProgress);
    expect(roomState.roundTimestamp).toBe(currentTime + Ludo.Config.RoundLength);
  });
  test('Player rolls 5 with 1 pawn on field', () => {
    ...
    expect(returnActions.length).toBe(3);
    const rollAction = returnActions[0];
    const waitAction = callbackActions[1];
