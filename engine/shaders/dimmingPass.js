...
const getPrepareMaskMaterial = () => new THREE.ShaderMaterial({
  uniforms: {
...
const getEdgeDetectionMaterial = () => new THREE.ShaderMaterial({
...
const getSeperableBlurMaterial = maxRadius => new THREE.ShaderMaterial({
...
const getOverlayMaterial = () => new THREE.ShaderMaterial({
...
class DimmingPass extends THREE.Pass {
  constructor(resolution, scene, camera, selectedObjects, options = {}) {
    super();
    ...
    // copy material
    if (THREE.CopyShader === undefined) { throw new Error('THREE.OutlinePass relies on THREE.CopyShader'); }
  ...
  render(renderer, writeBuffer, readBuffer, delta, maskActive) {
    if (this.selectedObjects.length > 0) {
      ...
      if (this.pulsePeriod > 0) {
        const scalar = (1 + 0.25) / 2 + Math.cos(window.performance.now() * 0.01 / this.pulsePeriod) * (1.0 - 0.25) / 2;
