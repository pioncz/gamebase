...
export default class Pawn {
  ...
  createSelectionObject() {
    ...
    this.selectionObject = new THREE.Mesh(
      selectionGeometry,
      selectionMaterial,
    );
  ...
  select(orientation) {
    ...
    this.context.animations
      .create({
        ...
        update: (progress) => {
          this.selectionObject.position.y = 3.2 - progress * 0.6;
          this.selectionObject.material.opacity = Math.min(
            progress * 3,
            1.0,
          );
      ...
      .then(() => {
        this.context.animations.create({
          ...
          update: (progress) => {
            ...
            if (progress >= 0.5) {
              parsedProgress = 0.5 - (progress - 0.5);
            }
            this.selectionObject.position.y =
              2.6 + parsedProgress * 0.6;
            this.glowMesh.material.uniforms.i.value =
              0.85 + parseInt(parsedProgress * 100) / 100;
          },
        });
      });
  }
  unselect() {
    ...
    this.context.animations.removeAnimation(
      'pawnAnimation' + this.id,
    );
    if (this.selectionObject) {
      this.selectionObject.material.opacity = 0;
    }
  ...
  darken() {
    if (
      this.context.animations._getAnimationById(
        `pawn ${this.id} darken`,
      )
    ) {
      return;
    ...
    this.context.animations.create({
      ...
      update: (progress) => {
        ...
        this.pawnMesh.material.color.setHSL(
          colorHSL.h,
          colorHSL.s,
          light,
        );
  ...
  normalizeColor() {
    ...
    const colorHSL = this.color.getHSL(new THREE.Color());
    const materialHSL = this.pawnMesh.material.color.getHSL(
      new THREE.Color(),
    );
    this.context.animations.create({
      ...
      update: (progress) => {
        const light =
          materialHSL.l * (1 - progress) + colorHSL.l * progress;
        this.pawnMesh.material.color.setHSL(
          colorHSL.h,
          colorHSL.s,
          light,
