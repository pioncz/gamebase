...
import { Room, RoomStates } from './../room/index.js';
import Games from '../../games/Games.js';
import ActionsStream from './../actions-stream/index.js';
import BotsManager from './../bots-manager/index.js';
...
class WebsocketServer {
  constructor(io, playerService) {
    ...
    let _getTotalNumPlayers = () => {
      ...
      _leaveGame = (socketId) => {
        let connection = this.connections[socketId],
          ...
          playerIndex =
            room &&
            room.gameState.players.findIndex(
        ...
        if (
          !room.gameState.playerIds.length ||
          room.gameState.roomState === RoomStates.finished
        ) {
      ...
      _destroyConnection = (socketId) => {
        let connection = this.connections[socketId],
          player =
            connection.playerId && this.players[connection.playerId];
      ...
      _findRoom = (gameName) => {
        ...
        const matchingRooms = Object.keys(this.rooms).reduce(
          (returnRooms, roomId) => {
            const room = this.rooms[roomId];

            if (
              room.gameState.gameName === gameName &&
    ...
    const _handleFindRoom = (socket) => (options) => {
      ...
      const minPlayers = Games[gameName].Config.MinPlayer;
      _log(
        `Player ${player.login} joins queue(${room.gameState.playerIds.length}/${minPlayers}) in ${room.name}`,
      );
    ...
    const _handleCallAction = (socket) => (action) => {
      ...
      _log(
        `Player: ${player.login} calls action: ${JSON.stringify(
          action,
        )}`,
      );
    ...
    const _handleJoinRoom = (socket) => (options) => {
      ...
      if (room) {
        if (room.id === roomId) {
          _log(
            'Player tried to join same room second time, roomState emitted',
          );
          _emitRoomState(room);
        } else {
    ...
    io.use((socket, next) => {
      ...
      if (token) {
        ...
        playerPromise
          .then((playerId) => {
            return playerService.getById(playerId).then((player) => {
              updatePlayer(player);
            }, createTempPlayer);
          })
    ...
    io.on('connection', (socket) => {
      const player =
        this.players[this.connections[socket.id].playerId];
      _log(
        `New connection {sockeId: ${socket.id}, legin: ${
          player.login
        }, temp: ${
          player.temporary
  ...
  closeRoom(roomId) {
    ...
    const socketIds = room.gameState.players.map(
  ...
  emitRoomAction(room, action) {
    ...
    const roomBots = room.gameState.players.filter(
      (player) => !!player.bot && !!player.handleAction,
    );
    ...
    for (let bot of roomBots) {
      ...
      if (botActions.length) {
        const delay =
          RandomDelays[0] +
          (RandomDelays[1] - RandomDelays[0]) * Math.random();
        botActions = botActions.map((action) => ({
          ...action,
          timestamp: action.timestamp + delay,
        }));
        _log(
          botActions.map((action) => ({
            ...action,
            startEpoch:
              action.timestamp - room.gameState.startGameTimestamp,
          })),
        );
        this.emitRoomActions(room.name, botActions);
      }
    }
  }
